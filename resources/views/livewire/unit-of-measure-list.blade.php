                                ...
                                <thead class="text-sm bg-sky">
                                    <tr>
                                        ...
                                        <th class="text-center col-1">
                                            <a href="{{ route('maintenanceinventoryunit_of_measure_create') }}"
                                                class="text-white"> <i class="fas fa-plus"></i></a>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody class="text-sm">
                                    @foreach ($unitOfMeasure as $list)
                                        <tr>
                                            <td> {{ $list->NAME }}</td>
                                            <td> {{ $list->SYMBOL }}</td>
                                            <td> {{ $list->INACTIVE ? 'Yes' : 'No' }}</td>

                                            <td class="text-center">
                                                <a href="{{ route('maintenanceinventoryunit_of_measure_edit', ['id' => $list->ID]) }}"
