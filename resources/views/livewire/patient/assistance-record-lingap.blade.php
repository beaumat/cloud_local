    <table class="table table-sm table-bordered table-hover">
        <thead class="bg-sky text-xs">
            <tr>
                ...
                <th class="text-right col-1 bg-danger">Balance <i wire:click='reload()' type="button" class="fa fa-refresh" aria-hidden="true"></th>
            </tr>
        </thead>
        <tbody class="text-xs">
            @foreach ($dataList as $list)
                ...
                <tr>
                    ...
                    <td>{{ $list->DEPOSIT_AMOUNT > 0 ? date('m/d/Y', strtotime($list->TRANS_DATE)) : '' }}</td>
                    <td class="text-right">
                        {{ $list->DEPOSIT_AMOUNT > 0 ? number_format($list->DEPOSIT_AMOUNT, 2) : '' }}</td>
                    <td>
                        @if ($list->DEPOSIT_AMOUNT > 0)
                            <a target="_BLANK" href="{{ route('patientspayment_edit', ['id' => $list->TRANS_ID]) }}">
                                {{ $list->P_CODE }}
                            </a>
                        @endif
                    </td>
