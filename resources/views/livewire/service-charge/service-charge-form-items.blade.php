        ...
        <tbody class="text-xs">
            ...
            <tr class="@if ($list->other_charge) text-success font-weight-bold active @endif">
                ...
                <td class="text-right">
                    ...
                    <input type="number" step="0.01" class="form-control form-control-sm text-right" @if(!$canBeQtyEdit)
                        disabled @endif name="lineQty" wire:model.live.debounce.1000ms='lineQty'
                ...
                <td class="text-center">
                    ...
                    <button title="Delete Active" id="deletebtn" wire:click='deleteItem({{ $list->ID }})'
                        wire:confirm="Are you sure you want to delete this?" class="btn btn-xs btn-danger">
                        <i class="fas fa-trash" aria-hidden="true"></i>
                    </button>

                    @else
                    <button type="button" title="Edit Disabled" id="editbtn" class="btn btn-xs btn-secondary">
                        <i class="fas fa-edit" aria-hidden="true"></i>
                    </button>
                    <button type="button" title="Delete Disabled" id="deletebtn" class="btn btn-xs btn-secondary">
                        <i class="fas fa-trash" aria-hidden="true"></i>
                    </button>
                    @endif
                    {{-- Other Buttons --}}
                    <button class="btn btn-xs btn-primary" title="Cash Payment"
                        wire:click="cashPayment({{ $list->ID }}, {{ $list->AMOUNT }})"> <i class="fa fa-money"
                            aria-hidden="true"></i> </button>
                    <button class="btn btn-xs btn-success" title="Open Payment"
                        wire:click="openPayment({{ $list->ID }}, {{ $list->AMOUNT }})"> <i class="fa fa-paypal"
                            aria-hidden="true"></i> </button>
                    @endif
                </td>
                {{-- @endif --}}
                ...
                <tr class="text-xs">
                    ...
                    <td>
                        <input type="number" step="0.01" class="form-control form-control-sm text-xs mt-2 text-right"
                            name="Qty" wire:model.live.debounce.1000ms='QUANTITY' wire:blur="getAmount" @if($ITEM_ID==0)
                            readonly @endif />
                    </td>
                    <td>
                    ...
                    <td>
                        <input type="number" step="0.01" class="form-control form-control-sm mt-2 text-xs text-right"
                            @if ($editPrice==false) readonly @endif name="rate" wire:model.live.debounce.1000ms='RATE'
                            wire:blur="getAmount" />
                    </td>
                    <td class="text-right">
                    ...
                    <td>
                        <div class="mt-2">
                            <button type="submit" wire:loading.attr='hidden' @if ($ITEM_ID==0) disabled @endif
