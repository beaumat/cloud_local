                ...
                @for ($j = 0; $j < 7; $j++)
                        ...
                        <td>
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <b
                                        class="@if ($today == $year . '-' . str_pad($month, 2, '0', STR_PAD_LEFT) . '-' . str_pad($currentDay, 2, '0', STR_PAD_LEFT)) text-primary @endif text-sm">{{ $currentDay }}</b>
                    ...
                    @elseif ($isExtraDay)
                        <td style="background-color: whitesmoke;">
                            ...
                            @if ($month == 12)
                                @livewire('Scheduler.shift', ['id' => $year + 1 . '-' . $month - 11 . '-' . $dayCounter++, 'contact_id' => $CONTACT_ID, 'shiftList' => $shiftList, 'location_id' => $LOCATION_ID])
                            @else
                                @livewire('Scheduler.shift', ['id' => $year . '-' . $month + 1 . '-' . $dayCounter++, 'contact_id' => $CONTACT_ID, 'shiftList' => $shiftList, 'location_id' => $LOCATION_ID])
                            @endif
