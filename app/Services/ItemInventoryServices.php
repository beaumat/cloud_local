...
class ItemInventoryServices
{
    ...
    CASE 
		...
		WHEN document_type_map.`ID` = 19 THEN  (SELECT build_assembly.`NOTES` FROM build_assembly  WHERE build_assembly.`ASSEMBLY_ITEM_ID` =  item_inventory.`SOURCE_REF_ID` AND build_assembly.`DATE` = item_inventory.`SOURCE_REF_DATE` AND build_assembly.`LOCATION_ID` = item_inventory.`LOCATION_ID` AND build_assembly.`ASSEMBLY_ITEM_ID` = item_inventory.`ITEM_ID` )			
