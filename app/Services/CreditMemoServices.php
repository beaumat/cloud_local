...
class CreditMemoServices
{
    ...
    {
        return CreditMemo::where('ID', $ID)->first();
    }
    ...
    public function ItemStore(
        ...
        $LINE_NO = $this->getLine($CREDIT_MEMO_ID) + 1;
    ...
    public function ItemUpdate(
        ...
        CreditMemoItems::where('ID', $ID)->where('CREDIT_MEMO_ID', $CREDIT_MEMO_ID)->where('ITEM_ID', $ITEM_ID)->update([
            'QUANTITY' => $QUANTITY,
            'UNIT_ID' => $UNIT_ID > 0 ? $UNIT_ID : null,
            'UNIT_BASE_QUANTITY' => $UNIT_BASE_QUANTITY,
            'RATE' => $RATE,
    ...
    public function Search($search, int $locationId, int $perPage)
    {
        return CreditMemo::query()
            ...
            ->when($search, function ($query) use (&$search) {
                $query->where('credit_memo.CODE', 'like', '%' . $search . '%')
