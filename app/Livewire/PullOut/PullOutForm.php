...
class PullOutForm extends Component
{
    ...
    public function save()
    {
        ...
        try {
            if ($this->ID == 0) {
                $this->ID = $this->pullOutServices->Store($this->CODE, $this->DATE, $this->LOCATION_ID, $this->NOTES, $this->PREPARED_BY_ID, $this->ACCOUNT_ID);
                DB::commit();
                return Redirect::route('companypull_out_edit', ['id' => $this->ID])->with('message', 'Successfully created');
            } else {

                $this->pullOutServices->Update($this->ID, $this->CODE, $this->NOTES, $this->PREPARED_BY_ID);
                DB::commit();
